...
import { acceptWorkflowProposal, analyzeWorkflows, attachSessionTerminal, createWorkflow, deleteWorkflow, focusSessionPane, dismissWorkflowProposal, getRepoConfig, getRepos, getSessionTerminalUrl, getTmuxInventory, getWorkflowAnalysis, getWorkflowCatalog, getWorkflowProposals, listLocalSkills, readLocalSkill, saveRepoSkillAssociations, updateWorkflow, WorkflowSaveError } from "./api";
...
import { adoptServerConfig, cancel as cancelDraft, createWorkflowWorkspaceDraft, setJsonText, setStages, switchView, type DraftView, type WorkflowDraftState } from "./components/workflow-draft";
...
function WorkflowWorkspace({ insights, selectId, onLaunch }: { insights: WorkflowInsights; selectId?: string; onLaunch: () => void }) {
  ...
  const [creating, setCreating] = useState(false); const [deleting, setDeleting] = useState(false); const [newName, setNewName] = useState(""); const [rangeOpen, setRangeOpen] = useState(false); const [from, setFrom] = useState(() => isoDay(-14)); const [to, setTo] = useState(() => isoDay());
  ...
  useEffect(() => { const item = catalog.find((candidate) => candidate.id === id); if (item) setDraft(createWorkflowWorkspaceDraft(item.config, draft?.view)); }, [catalog, id]);
  ...
  async function remove() { if (!current) return; setMessage(null); try { await deleteWorkflow(current.id); setCatalog((items) => { const next = items.filter((item) => item.id !== current.id); setId(next[0]?.id ?? ""); return next; }); setDeleting(false); } catch (error) { const e = error as WorkflowSaveError; setMessage(e.code === "WORKFLOW_LAST" ? "no se puede eliminar el único workflow" : e.message); setDeleting(false); } }
  ...
  const deleteModal = deleting && current ? <div className="ronin-inline-modal"><div><h2>Eliminar workflow</h2><p>¿Eliminar <strong>{current.name}</strong>?</p><p className="ronin-form-note">Las sesiones ya creadas conservan su copia del workflow.</p><footer><button className="n-btn n-btn-secondary" onClick={() => setDeleting(false)}>Cancelar</button><button className="n-btn n-btn-danger" onClick={() => void remove()}>Eliminar</button></footer></div></div> : null;
  const headerActions = <><button className="n-btn n-btn-secondary" onClick={() => setCreating(true)}>＋ Nuevo</button><button className="n-btn n-btn-danger" disabled={catalog.length <= 1} onClick={() => setDeleting(true)}>Eliminar</button><button className="n-btn n-btn-secondary" onClick={() => setRangeOpen((open) => !open)}>✦ Analizar flujo reciente</button></>;
