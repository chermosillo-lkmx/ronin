  ...
  return (
    <div className="ronin-modal-backdrop" role="presentation" onMouseDown={onClose}>
      <form className="ronin-new-session" onSubmit={submit} onMouseDown={(event) => event.stopPropagation()}>
        ...
        <p className="ronin-form-note">{mode === "workflow" ? <>{request.trim() ? "Claude recibirá el workflow y esta petición al arrancar." : "Sin petición: tendrás que instruir al worker en la terminal."} Se crea una rama <code>ronin/&lt;sesión&gt;</code> desde la rama por defecto del repositorio y se congela el workflow elegido.</> : <>Se abre <code>{agent}</code> en la raíz del repositorio, sin crear rama, worktree ni workflow. Arrastra sobre el texto para copiarlo al portapapeles.</>}</p>
