...
export interface ClaudePOptions { timeoutMs?: number; maxBytes?: number; command?: string; args?: string[]; cwd?: string }
...
export function runClaudeP(input: string, opts: ClaudePOptions = {}): Promise<string> {
  const { timeoutMs = 120_000, maxBytes = 4 << 20, command = "claude", args = ["-p"], cwd } = opts;
  return new Promise((resolve, reject) => {
    const child = spawn(command, args, { stdio: ["pipe", "pipe", "pipe"], ...(cwd ? { cwd } : {}) });
